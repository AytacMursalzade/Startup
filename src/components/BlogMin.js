...
function BlogMin() {
    ...
    return (
        <div className="flex flex-wrap px-[70px] mt-[70px] ">
            {data.map((item) => (
                <div key={item.id} className="w-1/3 p-4 relative">
                    <div
                    ...
                    >
                        <div className="w-[428px] h-[255px] absolute inset-4 mt-[1px] bg-black opacity-0 hover:opacity-50 transition-opacity duration-300"></div>
                    </div>
                </div>
            ))}
        </div>
